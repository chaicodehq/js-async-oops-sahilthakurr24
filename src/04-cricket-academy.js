...
export class Batsman extends Player {
  ...
  getStrikeRate() {
    ...
    const avgSR =
      this.innings.reduce((acc, curr) => {
        const sr = (curr.runs / curr.balls) * 100;
        return acc + sr;
...
export class Bowler extends Player {
  ...
  getEconomy() {
    ...
    const avgEco =
      this.spells.reduce((acc, curr) => {
        return acc + curr.runsConceded / curr.overs;
